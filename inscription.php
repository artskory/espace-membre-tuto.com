<?php 
    ...
    if (!empty( $_POST ) ) {
        ...
        if ( empty( $email ) ) {
            $erreur['email'] = 'Adresse e-mail maquante';
        }
        elseif ( !filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
            $erreur['email'] = 'Adresse e-mail invalide';
        }
        elseif ( !mail_free() ) {
            $erreur['email'] = 'Adresse e-mail déja existante';
        }
        
        if ( empty( $password ) ) {
            $erreur['password'] = 'Mot de passe maquant';
        }
        ...
        if ( empty( $passwordconf ) ) {
            $erreur['passwordconf'] = 'Confirmation du mot de passe maquante';
        }
